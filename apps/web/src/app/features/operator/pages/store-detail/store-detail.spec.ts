...
import type { OperatorStore } from '../../../../core/services/operator-store.service';
import { OperatorStoreDetail } from './store-detail';

const API = 'http://localhost:3000/api/v1';

function store(overrides: Partial<OperatorStore> = {}): OperatorStore {
  return {
    id: 'store-1',
...
describe('OperatorStoreDetail', () => {
  ...
  async function open(overrides: Partial<OperatorStore> = {}) {
  ...
  function saveButton(): HTMLButtonElement {
    return el.querySelector<HTMLButtonElement>('.terms-actions .btn-ink')!;
  }
  ...
  it('shows the before and the after only once something actually changed', async () => {
    ...
    const [before, after] = Array.from(rows[0].querySelectorAll('td.num')).map((td) =>
      td.textContent!.trim(),
    );
  ...
  it('refuses a value outside the range the entity accepts, before asking the API', async () => {
    ...
    expect(el.querySelector('.field-error')?.textContent).toContain('fora da faixa');
    expect(saveButton().disabled).toBe(true);
  ...
  it('puts the form back where it was when the change is discarded', async () => {
    ...
    el.querySelector<HTMLButtonElement>('.terms-actions .btn-quiet')!.click();
    await settle();
