...
import type { OperatorAuditLog } from '../../../../core/services/operator-audit.service';
...
describe('OperatorAuditLogPage', () => {
  ...
  it('reads a LIVE decision as a sentence, with the reason it carried', async () => {
    ...
    expect(row.querySelector('.chip')?.textContent?.trim()).toBe('Aprovou LIVE');
    expect(row.querySelector('.chip')?.getAttribute('data-tone')).toBe('ok');
  ...
  it('shows an investigation as a line with no before and after', async () => {
    ...
    expect(el.querySelector('.chip')?.textContent?.trim()).toBe('Abriu para investigar');
  ...
  it('narrows the trail to the operator asking for it', async () => {
    ...
    expect(el.querySelector('app-page-state')?.textContent).toContain('Você ainda não decidiu');
  });
  ...
  it('walks by offset, like the queue, because there is no total to page against', async () => {
    ...
    const [previous, next] = Array.from(
      el.querySelectorAll<HTMLButtonElement>('.trail-foot-actions button'),
    );
    ...
    expect(el.querySelectorAll<HTMLButtonElement>('.trail-foot-actions button')[1].disabled).toBe(
      true,
    );
  });
});
