...
const LIVE_STATUS_NOTE: Record<StoreLiveStatus, string> = {
  NOT_REQUESTED:
    'Cobrar em TEST nao depende de aprovacao nenhuma. LIVE passa pela mesa — e, aqui, LIVE tambem e simulado.',
  PENDING: 'A mesa recebeu o pedido. TEST continua funcionando normalmente.',
  APPROVED:
    'A key hk_live_ desta loja ja cobra e credita o ledger LIVE — que tambem e simulado, com dinheiro que nao existe.',
  REJECTED: 'Pedido recusado. Da para pedir de novo depois de resolver o motivo abaixo.',
  SUSPENDED: 'A mesa revogou a habilitacao. LIVE esta fechado; reabrir e decisao da mesa.',
};
...
export class Settings implements OnInit {
  ...
  readonly liveStatusNote = computed(() => LIVE_STATUS_NOTE[this.liveStatus()]);
  readonly canRequestLive = computed(
    () => this.liveStatus() === 'NOT_REQUESTED' || this.liveStatus() === 'REJECTED',
  );
  readonly isRequestingLive = signal(false);
  ...
  requestLive(): void {
    ...
    if (!store || !this.canRequestLive()) return;

    this.isRequestingLive.set(true);
    this.storeService.requestLiveEnablement(store.id).subscribe({
      next: () => this.isRequestingLive.set(false),
      error: (err) => {
        this.error.set(
          err?.error?.error?.message || err?.message || 'Erro ao pedir habilitacao LIVE.',
        );
        this.isRequestingLive.set(false);
      },
    });
  }
