...
import type { AdminStoreListItem } from '../../services/operator-store.service';
import { OperatorQueue } from './queue';

const API = 'http://localhost:3000/api/v1';

function store(overrides: Partial<AdminStoreListItem> = {}): AdminStoreListItem {
  return {
    id: 'store-1',
...
describe('OperatorQueue', () => {
  ...
  async function open(url = '/operator', data: AdminStoreListItem[] = [store()]) {
  ...
  function sheet() {
    return el.querySelector('adm-sheet');
  }
  ...
  it('sends the decision with its reason and shows the new state on the row', async () => {
    ...
    expect(el.querySelector('tbody tr .adm-chip')?.textContent?.trim()).toBe('Aprovada');
    expect(el.querySelector('tbody tr .reason')?.textContent?.trim()).toBe('Documentos conferem.');
    expect(el.querySelector('adm-sheet')).toBeNull();
  });

  it('says the queue is empty rather than showing an empty table', async () => {
    await open('/operator', []);

    expect(el.querySelector('adm-page-state')?.getAttribute('data-variant')).toBe('empty');
    expect(el.querySelector('adm-page-state')?.textContent).toContain(
