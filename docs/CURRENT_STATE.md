...
| `apps/web`                | Angular unico para landing, auth, dashboard do merchant e mesa de operador. O dashboard tem overview, payments, Payment Links, products, receipts, customers, API keys, webhooks, alerts, financials, withdrawals e settings de perfil (`name`, `city`), com seletor TEST/LIVE na topbar. A mesa vive sob `/operator`, com guard, layout e sessao proprios: fila, decisao, condicao comercial, trilha e investigacao de loja, e -- de dentro da investigacao -- saque pela loja na aba de saldo e estorno pela linha do tempo do pagamento. O admin tem design system proprio em `app/admin/ui` (tokens em `admin.css`, tema claro/escuro, densidade e paleta de comandos) e nao usa biblioteca de UI de terceiro. |
...
| Superficie de operador            | Implementado         | Fronteira, trilha, tela e **quatro poderes**: habilitar loja para LIVE, mudar condicao comercial, ler dado de loja para investigar e mover dinheiro pela loja (saque e estorno, por chamado, pela tela da investigacao). Principal `Operator` com tabela, cookie e segredo proprios, sessao, `/operator/me`, trilha append-only e `/operator/stores` (fila, decisao, condicao comercial e as cinco leituras). A mesa tem tela propria em `apps/web` sob `/operator`. Sem papeis dentro do operador, sem impersonacao e sem fila de revisao antifraude. |
...
| Operador          | `operator-auth`, `operator`, `operator-store`, `operator-store-read`, `operator-store-money` | `Operator`, `OperatorRefreshToken`, `OperatorAuditLog`, `Store.liveStatus`, `Store.feePercent`/`feeFixed`/`settlementDays` | mesa em `/operator` (fila, detalhe de loja, condicao comercial, investigacao, saque e estorno pela loja, e trilha); lojista pede LIVE em Settings | coberto por unit tests e pelo e2e da API; sem smoke dedicado | Quatro poderes: habilitar LIVE, condicao comercial, leitura para investigar (inclusive os destinos Pix da loja, sem ambiente) e mover dinheiro pela loja. Sem papeis, sem impersonacao e sem antifraude; fila e trilha paginam por `offset`/`limit` e nao devolvem total; provisionamento por `pnpm operator:create`. |
...
- Trilha de auditoria (`operator_audit_logs`) e append-only: a porta nao tem update nem delete, e o repositorio so existe dentro do `UnitOfWork`, entao a linha e escrita na mesma transacao da mudanca que descreve. Hoje registra nove acoes, todas com `requestId`: `operator.login`, `operator.logout`, `store.live_approved|rejected|suspended`, `store.commercial_terms_changed`, `store.investigated`, `store.withdrawal_created` e `store.refund_created`. As tres primeiras de loja e a de condicao comercial carregam `before`/`after` e o motivo; as duas de dinheiro tambem, com o saldo (saque) ou o estado do pagamento (estorno) dos dois lados. Sem retencao ou purga.
