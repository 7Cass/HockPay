...
| `apps/web`                | Angular unico para landing, auth, dashboard do merchant e mesa de operador. O dashboard tem overview, payments, Payment Links, products, receipts, customers, API keys, webhooks, alerts, financials, withdrawals e settings de perfil (`name`, `city`), com seletor TEST/LIVE na topbar. A mesa vive sob `/operator`, com guard, layout e sessao proprios: fila, decisao, condicao comercial, trilha e investigacao de loja. |
