...
export class ReceiveWebhookInboxEventUseCase {
  ...
  async execute(input: IReceiveWebhookInboxEventInput): Promise<IReceiveWebhookInboxEventOutput> {
    ...
    const event = WebhookInboxEvent.create({
      ...
      outboxEventId: typeof input.payload.id === 'string' ? input.payload.id : undefined,
