...
import { CreatePaymentUseCase, PaymentCustomerInput } from './create-payment.use-case';
...
export class FulfillCheckoutSessionUseCase {
  ...
  async execute(input: IFulfillCheckoutSessionInput): Promise<IFulfillCheckoutSessionOutput> {
    ...
    if (session.status !== 'OPEN') {
      throw new Error(`Checkout session cannot be fulfilled because its status is ${session.status}`);
    }
