...
export class CreateBankAccountUseCase {
    ...
    async execute(dto: CreateBankAccountDto, merchantDocument: string): Promise<BankAccount> {
        ...
        if (cleanHolderDoc !== cleanMerchantDoc) {
            throw new Error(`Holder document (${dto.holderDocument}) does not match merchant document (${merchantDocument}). Third-party accounts are not allowed.`);
        }
        ...
        await this.unitOfWork.execute(async (repos) => {
            ...
            if (bankAccount.isDefault) {
                await repos.bankAccountRepository.clearDefaultFlagExcept(bankAccount.storeId, bankAccount.id);
            }
        });
