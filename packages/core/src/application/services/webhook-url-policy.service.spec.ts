...
describe("webhook URL policy", () => {
  it.each([
    ...
    "https://api.partner.example/webhooks/hockpay",
  ...
  it.each(["http://localhost:3999/webhook", "http://127.0.0.1:3999/webhook"])(
    ...
    (url) => {
      expect(validateWebhookUrl(url, { allowLocalHttp: true }).valid).toBe(
        true,
      );
      expect(validateWebhookUrl(url, { allowLocalHttp: false }).valid).toBe(
        false,
      );
    },
  );
