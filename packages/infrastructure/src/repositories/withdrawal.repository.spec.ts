...
describe('WithdrawalRepository', () => {
  ...
  it('does not claim withdrawals when limit is zero or negative', async () => {
    ...
    await expect(repository.claimProcessableWithdrawals({ limit: 0 })).resolves.toEqual([]);
    await expect(repository.claimProcessableWithdrawals({ limit: -1 })).resolves.toEqual([]);
